...
namespace InitializrApi.Services
{
    public class InitializrTemplateCache
    {
        ...
        public InitializrTemplateCache(IEngineEnvironmentSettings environmentSettings)
        {
        ...
        public InitializrTemplateCache(IEngineEnvironmentSettings environmentSettings, List<TemplateInfo> templatesInCache)
            : this(environmentSettings)
        {
            TemplateInfo = templatesInCache;
        }

        public InitializrTemplateCache(IEngineEnvironmentSettings environmentSettings, JObject parsed, string cacheVersion)
            : this(environmentSettings)
        {
            ...
            if (contentJobject.TryGetValue("TemplateInfo", StringComparison.OrdinalIgnoreCase, out JToken templateInfoToken))
            {
                if (templateInfoToken is JArray arr)
                {
                    foreach (JToken entry in arr)
                    {
                        ...
                        {
                            templateList.Add(Microsoft.TemplateEngine.Edge.Settings.TemplateInfo.FromJObject((JObject)entry, cacheVersion));
                        }
                    }
                }
            }
